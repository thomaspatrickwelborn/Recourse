...
// Object Getter
function Getter$1($target, $property, $options) {
...
const Options$e = { getters: [Getters.Object] };
...
function entities($source, $type, $options) {
  ...
  new Cessors([Getters.Object]).cess($source);
...
const Options$5 = { ancestors: [] };
function freeze($target, $options) {
  const { ancestors } = Object.assign({}, Options$5, $options, {
...
const Options$4 = { ancestors: [] };
function seal($target, $options) {
  const { ancestors } = Object.assign({}, Options$4, $options, {
    ancestors: Object.assign([], $options.ancestors)
  });
  if(!options.ancestors.includes($target)) { options.ancestors.unshift($target); }
  iterateTargetProperties: 
...
const Options$1 = { getters: [Getters.Object], recurse: false };
function valueOf($source, $options) {
  const options = Object.assign({}, Options$1, $options);
  const getters = new Cessors(options);
  getters.cess($source, options);
  throw ["source", $source]
}

const Options = {
  getters: [Getters.Object], space: 0, replacer: null
};
...
export { Recourse, assign, assignConcat, index as cessors, compand, decompand, defineProperties, defineProperty, deleteProperty as delete, entities, entries, expand, freeze, getProperty as get, getOwnPropertyDescriptor, getOwnPropertyDescriptors, impand, isArrayLike, isEntries, keys, regularExpressions, seal, setProperty as set, splitPath, toString, typeOf, typedObjectLiteral, valueOf, values, index$1 as variables };
